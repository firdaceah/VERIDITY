...
@section('content')
    <div class="max-w-6xl mx-auto">
        <div class="mb-6">
            <a href="{{ route('user.my-audits') }}" class="text-blue-500 hover:text-blue-400 font-bold text-sm">
                ...
                <div class="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="p-4 bg-slate-950 rounded-2xl border border-slate-800">
                            ...
                            {{-- Baris Status Utama --}}
                            <p
                                class="text-xs font-bold {{ isset($analysis->metadata_details['summary']['status']) && str_contains($analysis->metadata_details['summary']['status'], 'Found') ? 'text-emerald-400' : 'text-orange-400' }}">
                                <i
                                    class="fa-solid {{ isset($analysis->metadata_details['summary']['status']) && str_contains($analysis->metadata_details['summary']['status'], 'Found') ? 'fa-circle-check' : 'fa-eye-slash' }} mr-1"></i>
                                {{ $analysis->metadata_details['summary']['status'] ?? 'No Data' }}
                            </p>

                            {{-- Tampilkan Detail Jika Ada (Ini yang bikin Merk Sony muncul) --}}
                            @if (isset($analysis->metadata_details['metadata']['camera']['Make']) ||
                                    isset($analysis->metadata_details['metadata']['camera']['Model']))
                                <div class="mt-2 pt-2 border-t border-slate-800/50 flex flex-col gap-1">
                                    <span class="text-[10px] text-blue-400 font-mono">
                                        Device: {{ $analysis->metadata_details['metadata']['camera']['Make'] ?? '' }}
                                        {{ $analysis->metadata_details['metadata']['camera']['Model'] ?? '' }}
                                    </span>
                                    @if (isset($analysis->metadata_details['metadata']['camera']['LensModel']))
                                        <span class="text-[9px] text-slate-500 italic">
                                            Lens: {{ $analysis->metadata_details['metadata']['camera']['LensModel'] }}
                                        </span>
                                    @endif
                                </div>
                            @endif
    ...
    <script>
        function switchView(type) {
            ...
            btnOriginal.classList.replace('bg-blue-600', 'bg-slate-800');
            btnEla.classList.replace('bg-blue-600', 'bg-slate-800');

            if (type === 'original') {
                imgElement.src = "{{ asset('storage/' . $analysis->s3_path) }}";
                btnOriginal.classList.replace('bg-slate-800', 'bg-blue-600');
            } else if (type === 'ela') {
                // AMBIL NAMA FILE UNIK DARI DATABASE
                ...
                btnEla.classList.replace('bg-slate-800', 'bg-blue-600');
            }
        }
    </script>
@endsection
