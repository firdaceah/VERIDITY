...
@section('styles')
    <style>
        ...
        .profile-wrap.admin { grid-template-columns:1fr; max-width:900px; }
        ...
        .logout-inline { background:var(--red-bg); color:var(--red); border:1px solid var(--red-border); border-radius:12px; padding:12px 16px; font-weight:900; cursor:pointer; }
    </style>
@endsection

@section('content')
    @if ($user->role === 'admin')
        <div class="profile-wrap admin">
            <div class="card">
                <div style="display:flex; gap:18px; align-items:center;">
                    ...
                    <div>
                        ...
                        <p style="font-size:13px; color:var(--muted); margin-top:4px;">{{ $user->email }} - ADMIN</p>
                    </div>
                </div>

                <div class="stats">
                    <div class="stat"><div style="font-size:24px; font-weight:900;">{{ $adminSummary['products'] }}</div><div style="font-size:11px; color:var(--muted);">Produk</div></div>
                    <div class="stat"><div style="font-size:24px; font-weight:900;">{{ $adminSummary['stores'] }}</div><div style="font-size:11px; color:var(--muted);">Toko Reseller</div></div>
                    <div class="stat"><div style="font-size:24px; font-weight:900;">{{ $adminSummary['orders_active'] }}</div><div style="font-size:11px; color:var(--muted);">Pesanan Aktif</div></div>
                    <div class="stat"><div style="font-size:24px; font-weight:900;">{{ $adminSummary['need_validation'] }}</div><div style="font-size:11px; color:var(--muted);">Perlu Validasi</div></div>
                </div>

                @if (session('success'))
                    <div style="background:var(--green-bg); border:1px solid var(--green-border); color:var(--green); padding:12px; border-radius:12px; margin-top:20px; font-size:13px; font-weight:800;">{{ session('success') }}</div>
                @endif

                <div style="margin-top:24px; display:grid; gap:12px;">
                    <div class="form-group"><label>Nama Admin</label><div class="form-control" style="background:var(--card);">{{ $user->name }}</div></div>
                    <div class="form-group"><label>Email</label><div class="form-control" style="background:var(--card);">{{ $user->email }}</div></div>
                    <div style="display:flex; gap:10px; flex-wrap:wrap;">
                        <a class="btn" href="{{ route('distri.profile.edit') }}">Edit Profile</a>
                        <a class="btn" href="{{ route('admin.orders.index') }}" style="background:var(--navy);">Pantau Pesanan</a>
                        <a class="btn" href="{{ route('admin.products.veridity') }}" style="background:var(--navy3);">Validasi Nota</a>
                    </div>
    ...
    @else
        <div class="profile-wrap">
            <div class="card">
                ...
                @endif
                <div style="margin-top:24px; display:grid; gap:12px;">
                    <div class="form-group"><label>Nama Toko / Pengguna</label><div class="form-control" style="background:var(--card);">{{ $user->name }}</div></div>
                    <div class="form-group"><label>Email</label><div class="form-control" style="background:var(--card);">{{ $user->email }}</div></div>
                    <div class="form-group"><label>Password</label><div class="form-control" style="background:var(--card);">Disembunyikan untuk keamanan</div></div>
                    <a class="btn" href="{{ route('distri.profile.edit') }}">Edit Profile</a>
                </div>
